...
use bytes::Bytes;
use digest::{Digest, FixedOutputReset, OutputSizeUser, generic_array::GenericArray};
...
impl<H: Digest + FixedOutputReset> TreeHasher<H> {
    ...
    pub(crate) fn digest(&mut self, data: impl AsRef<[u8]>) -> GenericArray<u8, <H as OutputSizeUser>::OutputSize> {
        <H as Digest>::update(&mut self.hasher, data);
        self.hasher.finalize_reset()
    }

    pub(crate) fn digest_leaf(&mut self, path: impl AsRef<[u8]>, leaf_data: impl AsRef<[u8]>) -> (GenericArray<u8, <H as OutputSizeUser>::OutputSize>, Vec<u8>) {
    ...
    pub(crate) fn digest_node(&mut self, left_data: impl AsRef<[u8]>, right_data: impl AsRef<[u8]>) -> (GenericArray<u8, <H as OutputSizeUser>::OutputSize>, Vec<u8>) {
    ...
    pub(crate) fn parse_leaf(data: &[u8]) -> (&[u8], &[u8]) {
        ...
        (&data[leaf_prefix_len .. path_size + leaf_prefix_len], &data[leaf_prefix_len + path_size .. ])
    }
    ...
    pub(crate) fn parse_node(data: &Bytes) -> (Bytes, Bytes) {
        ...
        (data.slice(node_prefix_len .. left_size + node_prefix_len), data.slice(node_prefix_len + left_size .. node_prefix_len + left_size + right_size))
    }
    ...
    pub(crate) fn path(&self, key: impl AsRef<[u8]>) -> GenericArray<u8, <H as OutputSizeUser>::OutputSize> {
